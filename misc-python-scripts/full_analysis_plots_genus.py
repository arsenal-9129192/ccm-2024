...
available_colors = [
    "lightcoral", "indianred", "firebrick", "red", "salmon", "coral", "orangered", "peru", "yellow", 
    "yellowgreen", "greenyellow", "chartreuse", "palegreen", "lightblue", "powderblue", "aqua", 
    "dodgerblue", "purple", "violet", "deeppink", "hotpink", "mediumvioletred", "blue", "slateblue", 
    "turquoise", "lightseagreen", "seagreen", "mediumpurple", "orange", "moccasin", "gold", "khaki", 
    "navajowhite"
]
...
genus_data = pd.read_csv('genus-feature-table-hellinger.tsv', sep='\t', skipinitialspace=True)
...
genus_data['#OTU ID'] = genus_data['#OTU ID'].str.replace('g__', '')  # Remover prefixo 'g__' para obter o nome do gênero
...
with pd.ExcelWriter('condition_rankings_genus.xlsx') as writer:
...
general_ranking.to_csv('general_ranking_genus.csv')
...
plt.savefig('stacked_bar_chart_genus.pdf', bbox_inches='tight')
plt.show()
...
for condition in grouped_normalized.index:
    ...
    plt.tight_layout()
    plt.savefig(f'pie_chart_{condition}_genus.pdf', bbox_inches='tight')
    plt.show()
...
plt.tight_layout()
plt.savefig('general_pie_chart_genus.pdf', bbox_inches='tight')
plt.show()
