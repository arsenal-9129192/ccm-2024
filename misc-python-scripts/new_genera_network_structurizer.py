        ...
        for line in infile:
            ...
            if stripped_line.startswith("node ["):
                ...
                if match_label:
                    ...
                    genera_match = re.search(r'g__([^;]+)', label)
                    phyla_match = re.search(r'p__([^;]+)', label)

                    genera = genera_match.group(1) if genera_match else ""
                    phyla = phyla_match.group(1) if phyla_match else ""

                    new_block = []
                    for node_line in node_block:
                        if "label" in node_line:
                            new_block.append(f'    label "{genera}"\n')
                            new_block.append(f'    domain "{phyla}"\n')
                        else:
...
input_file = 'spearman-genera-exported-network/network.gml'
output_file = 'spearman-genera-exported-network/spearman-genera-modified_network.gml'
