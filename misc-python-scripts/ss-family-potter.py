...
with pd.ExcelWriter('ss-condition_rankings_family.xlsx') as writer:
...
general_ranking.to_csv('ss-general_ranking_family.csv')
...
plt.savefig('ss-stacked_bar_chart_family.pdf', bbox_inches='tight')
plt.show()
...
for condition in grouped_normalized.index:
    ...
    plt.savefig(f'ss-pie_chart_family_{condition}.pdf', bbox_inches='tight')
    plt.show()
