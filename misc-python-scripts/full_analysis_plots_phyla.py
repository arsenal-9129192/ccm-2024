...
with pd.ExcelWriter('condition_rankings_phyla.xlsx') as writer:
...
general_ranking.to_csv('general_ranking_phyla.csv')
...
plt.savefig('stacked_bar_chart_phyla.pdf', bbox_inches='tight')
plt.show()
...
for condition in grouped_normalized.index:
    ...
    plt.tight_layout()
    plt.savefig(f'pie_chart_{condition}_phyla.pdf', bbox_inches='tight')
    plt.show()
...
plt.tight_layout()
plt.savefig('general_pie_chart_phyla.pdf', bbox_inches='tight')
plt.show()
