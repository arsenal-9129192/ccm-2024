...
grouped_data['Unassigned'] = grouped_data.filter(like='Unassigned', axis=1).sum(axis=1)
grouped_data['Unclassified'] = grouped_data.filter(like='Unclassified', axis=1).sum(axis=1)
grouped_data = grouped_data.drop(columns=grouped_data.filter(like='Unassigned').columns, errors='ignore')
grouped_data = grouped_data.drop(columns=grouped_data.filter(like='Unclassified').columns, errors='ignore')
grouped_percentage['Unassigned'] = grouped_percentage.filter(like='Unassigned', axis=1).sum(axis=1)
grouped_percentage['Unclassified'] = grouped_percentage.filter(like='Unclassified', axis=1).sum(axis=1)
grouped_percentage = grouped_percentage.drop(columns=grouped_percentage.filter(like='Unassigned').columns, errors='ignore')
grouped_percentage = grouped_percentage.drop(columns=grouped_percentage.filter(like='Unclassified').columns, errors='ignore')

with pd.ExcelWriter('gg-condition_rankings_genera_gpt.xlsx') as writer:
...
total_abundance['Unassigned'] = total_abundance.filter(like='Unassigned').sum()
total_abundance['Unclassified'] = total_abundance.filter(like='Unclassified').sum()
total_abundance = total_abundance.drop(labels=total_abundance.filter(like='Unassigned').index, errors='ignore')
total_abundance = total_abundance.drop(labels=total_abundance.filter(like='Unclassified').index, errors='ignore')
total_percentage['Unassigned'] = total_percentage.filter(like='Unassigned').sum()
total_percentage['Unclassified'] = total_percentage.filter(like='Unclassified').sum()
total_percentage = total_percentage.drop(labels=total_percentage.filter(like='Unassigned').index, errors='ignore')
total_percentage = total_percentage.drop(labels=total_percentage.filter(like='Unclassified').index, errors='ignore')

general_ranking = pd.DataFrame({
    'Abundance': total_abundance,
    'Percentage': total_percentage
}).sort_values(by='Percentage', ascending=False)
general_ranking.to_csv('gg-general_ranking_genera_gpt.csv')
...
plt.savefig('gg-stacked_bar_chart_genera_gpt.pdf', bbox_inches='tight')
plt.show()
